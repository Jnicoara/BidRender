...
- [ ] `references/backups.md` § "the cron itself" still instructs the reader to register the nightly backup with `manus-heartbeat create --cron "0 0 2 * * *"` from a Manus sandbox terminal. Every part of that is now wrong: the command does not exist, the six-field seconds-first format is Manus's and Cloudflare rejects it, and the time is stale since the move to `0 9 * * *`. Replace with the Cloudflare Worker sequence (`wrangler deploy`, then `wrangler secret put CRON_SECRET` — that order, because a secret cannot be attached to a Worker that does not exist yet, which is the opposite of what `workers/cron/README.md` used to say). Deliberately left alone on 2026-09-17 while the times were changed, because updating only the numbers would have made a wrong instruction look freshly checked.
- [ ] `references/deploying.md` § 7 says the cron "is created once, from a Manus sandbox terminal, after the site is deployed — a dev machine is unreachable from the platform". The second half is what makes this actively misleading: with Cloudflare it is the opposite, and the Worker is deployed from a local checkout with `wrangler`. It also cites `0 30 3 * * *`, six fields with seconds first. Same rewrite as the item above.
- [ ] Sweep the rest of `references/` for Manus-era platform instructions while doing the two above — these were found incidentally, not by looking, so there are probably more.
